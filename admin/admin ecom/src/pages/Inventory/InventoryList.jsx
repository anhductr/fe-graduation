...
export default function InventoryList() {
  ...
  const fetchStockInHistory = async ({ queryKey }) => {
    ...
    try {
      const res = await InventoryService.getStockInHistory({
        page,
        size,
        start: startDate, // Service expects ISO string or date, let's pass as is if service handles it or ensure ISO
        end: endDate,
      });
      // Note: startDate/endDate in queryKey seem to be ISO strings already as per line 136/137
      // But let's verify what `fetchStockInHistory` received.
      // Ah, queryKey has string. `InventoryService.getStockInHistory` takes params object.
  ...
  const deleteStockInMutation = useMutation({
    mutationFn: (referenceCode) =>
      InventoryService.deleteStockIn(referenceCode),
    onSuccess: () => {
  ...
  const handleDeleteStockIn = (referenceCode) => {
    deleteStockInMutation.mutate(referenceCode);
  };
  ...
  return (
    <>
                    ...
                    <TableRow key={inv.id}>
                      ...
                      <TableCell>
                        <Button
                          ...
                          className="!ml-auto !normal-case !bg-gradient-to-r !from-[#4a2fcf] !to-[#6440F5] !shadow"
                          onClick={() => {
                            handleOpenTransactionModal(inv.sku);
                          }}
                        >
      ...
      <Modal open={transactionModal} onClose={handleCloseTransactionModal}>
