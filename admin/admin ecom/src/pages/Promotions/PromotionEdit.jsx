  ...
  const editMutation = useMutation({
    ...
    onError: (err) => {
      if (err.response) {
        ...
        console.error(
          "🕓 Không nhận được phản hồi từ server. Request:",
          err.request
        );
      } else {
      ...
      navigate("/promotion", {
        state: {
          popup: {
            ...
            severity: "error",
            message:
              err.response?.data?.message || "Cập nhật sản khuyến mãi bại!",
            vertical: "top",
            horizontal: "center",
          },
        },
    ...
    if (applyTo !== defaultPromo.applyTo) {
      if (applyTo === "Category") {
        deleteApplyTo.current =
          defaultPromo.productId.length > 0
            ? Array.from(defaultPromo.productId)
            : [];
                            ...
                            <Chip
                              ...
                              onDelete={() => {
                                ...
                                setSelectedCategoriesId((prev) =>
                                  prev.filter((id) => id !== String(option.id))
                                );
                              }}
                              sx={{
              ...
              <div>
                <FormControl component="fieldset">
                  <RadioGroup
                    row
                    value={isVoucher}
                    onChange={(e) => setIsVoucher(e.target.value === "true")}
                  >
                    <FormControlLabel
                      value={false}
                      control={<Radio />}
