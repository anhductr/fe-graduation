...
export default function ContentList() {
  ...
  const fetchContents = async ({ queryKey }) => {
    ...
    // Construct params based on user request "limit=10&page=1"
    const params = {
      page: page,
      limit: size,
    };
    ...
    try {
      ...
      // If generic list (fallback or alternative structure)
      return {
        data: res.data.items || [],
  ...
  const deleteContent = async (contentId) => {
    const token = localStorage.getItem("token");
    await axios.delete(
      `${import.meta.env.VITE_CONTENT_CHATBOT_API_URL}/content/${contentId}`,
      {
        headers: {
          Authorization: token ? `Bearer ${token}` : "",
          "ngrok-skip-browser-warning": "true",
    ...
    <div className="py-[10px] px-[100px]">
      ...
      <div className="flex flex-wrap gap-[26px] w-full">
        ...
        <Boxes
          ...
          header={"Đã xuất bản"}
          total={
            contentData?.data?.filter((i) => i.status === "published").length ||
            0
          }
      ...
      <div className="shadow border-0 p-5 my-[20px] bg-white rounded-[10px]">
        ...
        {/* Filter section */}
        <div
          className={`${isToggleFilter ? "max-h-[200px] opacity-100 mt-4" : "max-h-0 opacity-0"} overflow-hidden transition-all duration-300 flex gap-4`}
        >
        ...
        {isLoading ? (
          ...
          contentData?.data?.map((item) => (
            <div
              ...
              className="shadow border-0 rounded-[10px] bg-white p-5 flex justify-between items-center"
            >
