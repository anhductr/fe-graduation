...
export default function InventoryModal({ onClose, initialData = null }) {
  ...
  useEffect(() => {
    if (initialData?.items && initialData.items.length > 0) {
      const loadedItems = initialData.items.map((item) => ({
        ...
        sku: item.sku, // Ensure SKU is loaded
        productId: item.productId, // Keep productId just in case
        productName: item.productName || item.sku, // Fallback to SKU if name missing
        selectedProduct: {
          id: item.productId,
  ...
  const handleSubmit = () => {
    ...
    const payload = {
      ...
      items: items
        .filter((item) => item.sku && item.quantity > 0 && item.unitCost >= 0)
        .map((item) => ({
          sku: item.sku,
          quantity: Number(item.quantity),
          unitCost: Number(item.unitCost),
          // Remove totalCost from item payload as per API requirement, only sku, quantity, unitCost
        })),
    };

    if (payload.items.length === 0) {
      alert("Không có sản phẩm hợp lệ (thiếu SKU hoặc số lượng)!");
      return;
    }
              ...
              <div className="grid grid-cols-2 w-full gap-4 px-1 auto-rows-max">
                <div className="flex gap-2">
                  ...
                  <input
                    ...
                    type="text"
                    className="w-[70%] bg-[#fafafa] pl-[15px] rounded-[5px] h-[40px] border-[rgba(0,0,0,0.1)] border border-solid"
                  ></input>
                </div>
                ...
                <div className="flex gap-2">
                  ...
                  <textarea
                    ...
                    className="w-[70%] bg-[#fafafa] pl-[15px] py-[10px] rounded-[5px] text-[15px] border-[rgba(0,0,0,0.1)] border border-solid"
                    rows={5}
                    cols={10}
                  ></textarea>
                </div>
