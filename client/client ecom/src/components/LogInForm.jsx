...
const LoginForm = ({ isOpen, onClose, onSwitchToRegister }) => {
  ...
  return (
    <>
      ...
      ></div>
      <div
        className="border border-gray-200 fixed z-50 top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white rounded-2xl w-96 shadow-lg"
        role="dialog"
        aria-modal="true"
        ...
        <form onSubmit={handleSubmit} className="px-6 pt-4 pb-6">
          ...
          {error && (
            <p className="text-red-600 text-sm mb-3">
              {loginError.response?.data?.message || "Đăng nhập thất bại"}
            </p>
          )}
