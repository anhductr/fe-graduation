...
export default function OrderPage() {
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const hasPending =
        res.data.result?.data?.some((order) => order.Status === "PENDING") ||
        false;
    ...
    <>
      <div className="flex flex-col gap-3">
        <div className="w-full flex justify-between items-center">
          <div className="text-[29px] font-bold">Đơn hàng của tôi</div>
        ...
        <div className="bg-white rounded-[10px] shadow w-full flex flex-col p-5 gap-4 pb-24">
          {active === "Chờ thanh toán" && (
            <div className="bg-orange-50 border border-orange-200 text-orange-800 px-4 py-3 rounded-md flex items-center gap-2 text-sm">
              <svg
                xmlns="http://www.w3.org/2000/svg"
              ...
              Nếu bạn không thanh toán trong vòng 24h đơn hàng sẽ bị hủy
            </div>
          )}
          {isLoading ? (
                  ...
                  {order.items?.slice(0, 2).map((item, idx) => (
                    <div key={idx} className="flex items-center gap-3">
                      ...
                      <p className="text-sm font-medium">
                        {formatPrice(
                          item.subTotal || item.sellPrice * item.quantity,
                        )}
                      </p>
                    </div>
                  ))}
